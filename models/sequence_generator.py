...
class SequenceGenerator(nn.Module):
    ...
    def _generate(
        ...
        #     def flush(self):
        #         pass
        # sys.stdout = Logger("model_architecture_seq_gen_model.txt")
        # print(model)
        # assert False
        ...
        #     # print(net_input['prev_output_tokens'])
        ...
        # print(encoder_outs[0]['encoder_out'])
        # assert False
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            # record the probs of each code
            if step!=0:
                target_code = sample["target"][0][step-1]
                tokens[0][step] = target_code
                # lprobs_counter += lprobs[0][target_code]
                # lprobs_counter_harmonic += 1. / lprobs[0][target_code]
                lprobs_counter_list.append(lprobs[0][target_code].item())

            # if step<3 and step!=0:
            #     print(step, lprobs.shape)
            #     print(tokens)
            #     tokens[0][step] = sample["target"][0][step-1]
            #     print(tokens)
            # elif step<3 and step==0:
            #     print(step, lprobs.shape)
            #     print(tokens)
            # else:
            #     print(step, lprobs.shape)
            ...
            # if step<3:
            #     print(step, cand_scores)
            #     # print(torch.max(lprobs[0], dim=0))
            #     print(lprobs[0][50265:58457].sum())
            #     print(scores)
            #     print(tokens)
            # else:
            #     print(step, cand_scores)
        ...
        # # print(finalized)
        # # calculate mean of log probability
        # lprobs_counter/=1024.0
        # probs_counter = lprobs_counter.item()
        # lprobs_counter = torch.log(lprobs_counter)
        # lprobs_counter -= torch.log(torch.tensor(1/8192.0))
        # # lprobs_counter = torch.exp(lprobs_counter)
        # # calculate harmonic mean of log probability
        # lprobs_counter_harmonic/=1024.0
        # lprobs_counter_harmonic = 1. / lprobs_counter_harmonic
        # lprobs_counter_harmonic = torch.log(lprobs_counter_harmonic)
        
        # lprobs_counter_harmonic -= torch.log(torch.tensor(1 / 8192.0))
        # print("sample_id: ", sample["id"], "| probs_counter: ", probs_counter,
        #       "| lprobs_counter: ", lprobs_counter.item(),
        #       "| lprobs_counter_harmonic: ", lprobs_counter_harmonic.item())
        # # print(scores[0][1023]/1024)
        # # assert False

        # score_filepath = "/home/qichen/Desktop/Evaluation/MyEvaluation/run_scripts/image_gen/toy_data/coco_karpathy_split/hard/scores.txt"
        # with open(score_filepath, "a") as f:
        #     f.write("{}\t{}\t{}\t{}\n".format(sample["id"][0], probs_counter, lprobs_counter.item(), lprobs_counter_harmonic.item()))
        ...
        # return finalized
        return finalized, lprobs_counter_list
