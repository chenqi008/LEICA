...
class DalleBartDecoder(nn.Module):
    ...
    def get_prob_list(self, settings, prev_tokens: LongTensor, **kwargs) -> Tuple[LongTensor, FloatTensor]:
        logits, _ = self.forward(prev_tokens=prev_tokens, **kwargs)
        image_count = logits.shape[0] // 2
        supercondition_factor = settings[[2]]
        logits = logits[:, -1, : 2 ** 14]
        logits: FloatTensor = (
        ...
        prob = logits[0][prev_tokens.cpu()[0].item()].cpu().item()
        return prob